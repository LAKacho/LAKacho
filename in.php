...
if(file_exists('Сотрудники.csv')){$lines = file('Сотрудники.csv');} else {exit();}
...
function dataRec($datas, $shifts) 
  ...
  $weekday=["воскресенье","понедельник","вторник","среда", "четверг", "пятница", "суббота"];
    ...
    if($diffDay<0){$diffDay = 4+$diffDay;}
  ...
  $pos = -1;
  $result = [];
  while (($pos = strpos($shiftA,$shifts,$pos+1))!==false) {$result[]=$pos;}
	...
	if(($result[2]%2)==0) { $meet=ceil(($result[2]-$result[0])/2); } else { $meet=ceil(($result[3]-$result[0])/2); }
    ...
    //print_r(" +day ".$meet);  echo "<br>";
  	...
  	if($nowDey=="воскресенье"||$nowDey=="суббота") {$dataMeet = date('d-m-Y', strtotime($dataMeet.'+4 day')); $nowDey = $weekday[date("w",strtotime($dataMeet))]; /*print_r($nowDey."новая встреча: ".$dataMeet." ".$nowDey."<br>" ); */ }
...
if(stripos($jobTitle[$line_num], "стажер")&&$shift<5&&!stripos($schedule[$line_num], "5-"))	
	...
	if  (stripos($arr[$line_num][31],"(")!==false){$cadet[$j]=1;} else {$cadet[$j]=0;} //стажеры и прочая полиция
	 
	 } else { $tabInternDiss[$j]=$tab[$line_num]; }                                                                         
                                                                                }   
   ...
   //$jobTitle[$line_num] = mb_convert_encoding($jobTitle[$line_num], 'windows-1251', 'utf-8');
	...
	foreach($resIntern as $row)        { $i++;
	$sqlIntern[$i] = (int)$row["tb_nomer"];
					              }         }

	
	if(!$sqlIntern){$sqlIntern[0]=0;}
	if(!$tabIntern){$tabIntern[0]=0;}    

    $diffDismIntern = array_diff($sqlIntern,$tabIntern); //ушедшие стажеры
	$diffNewIntern = array_diff($tabIntern,$sqlIntern); //новые стажеры
	

	echo "<br>ушедшие стажеры "; print_r($diffDismIntern);
	echo "-------------------------<br>";
	echo "<br>новые стажеры "; print_r($diffNewIntern);
...
// новая штатка
	 ...
	 if(stripos($intern[$key][2], "_")){$intern[$key][2]=str_replace("_","",$intern[$key][2]);}
	...
	foreach($diffDismIntern as $key => $value){
      ...
      //echo "arr[key][7]".$arr[$key][7]."----".$key."<br>";
	...
	foreach($tabIntern as $key => $value){
      ...
      $clock = explode(',', $timeIntern[$key]); if($clock[0]==""){$clock[0]=0;}
      $clock[0] = (int)$clock[0];
	...
	if(!isset($per)) {$per[0] = 0;} //если в sd таблице сотрудники нет значений
	...
	/*
	//$insert="INSERT INTO delme VALUES("..","..","..")";
	foreach ($lines as $line_num=> $line) {
	$arr[$line_num]= explode(";", $line);
	$fio = $arr[$line_num][1]." ".$arr[$line_num][2]." ".$arr[$line_num][3];
	$fio2 = $arr[$line_num][2]." ".$arr[$line_num][3];
  	
  	$fio2=mb_convert_encoding($fio2, 'windows-1251', 'utf-8');
  	$fio3=mb_convert_encoding($fio, 'windows-1251', 'utf-8');
  
  	if ($arr[$line_num][11]!="") { $i++;
	echo "Строка #<b>".$i."</b>: ".$arr[$line_num][0]." ".$fio." ".$arr[$line_num][11]."<br />\n";
	}
	//$insert="INSERT sd1 VALUES('B'"."'$arr[$line_num][0]'"."',12345,'".$i.")";
	$insert='INSERT sd1 VALUES("B'.$arr[$line_num][0].'","12345","'.$i.'")';
    $insert2='INSERT names_sd VALUES("B'.$arr[$line_num][0].'","'.$fio2.'","'.$fio3.'","'.$i.'")';
  	
  //$insert="INSERT sd1 VALUES('B444','12345','4')";
  if (mysqli_query($link, $insert)) {echo("OK <br>");} else {echo "Ошибка: ".mysqli_error($link);}
