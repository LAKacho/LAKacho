...
// Исправлено название таблицы sessions24
$sql1 = "SELECT user_login
         FROM xtvr_sessions24
...
if ($result1->num_rows > 0) {
    ...
    while ($row = $result1->fetch_assoc()) {
        ...
        // Используем тип "s" для строки вместо "i" в запросе
        ...
        $stmt->bind_param("s", $login); // Используем строку
        $stmt->execute();
        ...
        if ($email) {
            // Используем типы "s" для login и email
            ...
            $grade = 2;
            $insert_stmt->bind_param("ssi", $login, $email, $grade); // login и email как строки
            $insert_stmt->execute();
            $insert_stmt->close();
